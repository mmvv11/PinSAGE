...
class ItemToItemBatchSampler(IterableDataset):
    def __init__(self, g, user_type, item_type, batch_size):
        ...
        self.user_to_item_etype = list(g.metagraph()[user_type][item_type])[0] # user to item 엣지 타입
        self.item_to_user_etype = list(g.metagraph()[item_type][user_type])[0] # item to user 엣지 타입
        self.batch_size = batch_size

    def __iter__(self):
        while True:
            # 0~item node갯수 사이의 랜덤 정수
            heads = torch.randint(0, self.g.number_of_nodes(self.item_type), (self.batch_size,))
            # 그래프, 랜덤 워크 시작할 노드(head), 랜덤 워크에서 따르는 엣지 타입의 시퀀스
...
class NeighborSampler(object):
    ...
    def get_block(self, seeds, ntype, textset=None):
        '''
        :param seeds: item seeds list
        '''
        blocks = []
        for sampler in self.samplers:
            frontier = sampler(seeds)    
            ...
            blocks.insert(0, block)
        assign_features_to_blocks(blocks, self.g, textset, ntype)
